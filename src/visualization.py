...
def generate_sales_charts(data: pd.DataFrame) -> None:
    ...
    for i, txt in enumerate(sales):
        plt.annotate(f'{txt:.2f}', (months[i], sales[i]), textcoords="offset points", xytext=(0,10), ha='center')
    plt.grid(True)
    ...
    spending_amounts = [amount for _, amount in spending_data]  # Extract spending amounts
    plt.figure(figsize=(10, 6))
